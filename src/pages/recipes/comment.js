...
const Comment = (props) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const params = { recipeid: props.id };
        const queryString = new URLSearchParams(params).toString();
        const apiUrl = `http://192.168.27.1:1407/recipe_comment/data?${queryString}`;
  ...
  const submitComment = async () => {
    ...
    try {
      const response = await axios.post(
        `http://192.168.27.1:1407/recipe_comment`,
        {
          userid: userData.id,
          recipeid: props.id,
          commenttext: comment,
  ...
  const handleDeleteCmt = async ({ id }) => {
    ...
    try {
      const response = await axios.delete(
        `http://192.168.27.1:1407/recipe_comment/${id}`
      );
      if (response.status === 200) {
        const data = await response.data;
        console.log(data);
